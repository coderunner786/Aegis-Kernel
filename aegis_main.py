...
from web_server import start_dashboard_server, broadcast_telemetry
...
def trigger_interactive_alert(process_name: str, pid: int, score: float):
    """
    Fires an interactive Windows Toast Notification with Terminate and Ignore buttons.
    """
    ...
    <toast duration="long">
        <visual>
            <binding template="ToastGeneric">
                <text>⚠️ AEGIS ALERT: Threat Detected ({score:.1f}%)</text>
...
def make_event_handler(evaluator: AIEvaluator, db: AuditDatabase, enforcement_enabled: bool):
    def handle_event(event: SystemEvent):
        ...
        broadcast_telemetry(
            ...
            score=score * 100.0,
            cmd=event.cmdline or ""
        )

        # -------------------------------------------------------------
        # Threat Mitigation & Forensic Logging
        # -------------------------------------------------------------
        if is_anomaly:
...
def main():
    ...
    # 1. Start Web SOC Dashboard
    start_dashboard_server(port=8000)
    console.print("[bold green][*] Aegis SOC Dashboard live at http://localhost:8000[/bold green]")
